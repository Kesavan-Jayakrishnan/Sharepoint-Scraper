...
**Flow:**
1. Extract line-items table by column names (preferred) or range (fallback).
...
### UC-06: Doc AI without cell references
**Actor:** Platform Engineer / Auditor  
**Input:** Doc AI extracted content where exact sheet/cell is missing  
**Output:** Auditable semantic lineage

**Flow:**
1. Extraction stores best available locator in priority order (cell_ref -> named_range -> table_row_key -> heading/page/bbox -> chunk snippet hash).
2. Reconciliation still runs using extracted values.
3. Output lineage exposes `locator.type` and `locator.value` so reviewers can trace evidence even without strict cell refs.
4. If locator is unresolved, policy raises warning and routes for review.


- Column-name mapping is preferred for maintainability; fixed ranges are fallback when headers are unstable or missing.
